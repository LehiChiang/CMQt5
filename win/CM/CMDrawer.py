...
class CMDrawer(QWidget):
    ...
    def resizeEvent(self, event):
        ...
        super(CMDrawer, self).resizeEvent(event)
    ...
    def mousePressEvent(self, event):
        ...
        if pos.x() >= 0 and pos.y() >= 0 and self.childAt(pos) == None and self.widget:
            if not self.widget.geometry().contains(pos):
                self.animationOut()
                return
        super(CMDrawer, self).mousePressEvent(event)

    def show(self):
        super(CMDrawer, self).show()
